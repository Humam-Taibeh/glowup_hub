...
import { 
  ...
  Pause, RotateCcw, UserCircle, Clock, X, Moon, Heart, Brain, 
  ...
  Terminal, Repeat, GraduationCap, User as UserIcon, Undo2, LogOut, Shield,
  Fingerprint as IDIcon, Languages as LanguagesIcon, Maximize2
} from 'lucide-react';
      ...
      <div className="flex-1 overflow-hidden text-right" dir="rtl">
        <div className="flex items-center gap-2">
           ...
           {tk.deadline && tk.status !== 'done' && (
             <div className={`text-[8px] px-1.5 py-0.5 rounded-full font-black border-[0.5px] whitespace-nowrap ${isMissed ? 'bg-red-600/10 border-red-500/30 text-red-500 shadow-[0_0_8px_rgba(239,68,68,0.3)]' : 'bg-blue-600/10 border-blue-500/30 text-blue-500'}`}>
...
const App = () => {
  ...
  const glowX = useSpring(useTransform(mouseX, [0, 1920], [-50, 50]), { stiffness: 50, damping: 20 });
  const glowY = useSpring(useTransform(mouseY, [0, 1080], [-50, 50]), { stiffness: 50, damping: 20 });
      ...
      <div className="max-w-7xl mx-auto space-y-12 pb-40">
        ...
        <div className="flex flex-col md:flex-row gap-6 max-w-5xl mx-auto">
           ...
           <div className="flex gap-3 p-3 bg-white/[0.01] border-[0.5px] border-white/10 rounded-[2.5rem] backdrop-blur-xl">
              <select value={timeFilter} onChange={(e)=>setTimeFilter(e.target.value)} className="bg-transparent text-[10px] font-black uppercase text-zinc-500 outline-none cursor-pointer">
                  ...
                  <div className="space-y-16">
                    ...
                    <div className="space-y-8">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {SYSTEM_LANGS.map(lang => (
                          <button key={lang.id} onClick={async () => {
                            if(user.isDemo) { setProfile({...profile, lang: lang.id}); return; }
                            await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'core'), { lang: lang.id });
                          }} className={`p-8 rounded-[2rem] font-black text-xs transition-all border-[0.5px] flex items-center justify-between px-10 ${profile.lang === lang.id ? 'border-red-600 text-white bg-red-600/5 shadow-[0_0_20px_rgba(220,38,38,0.1)]' : 'border-white/5 text-zinc-700 bg-white/[0.01] hover:bg-white/5'}`}>
                    ...
                    <div className="space-y-8">
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        {UI_SCALES.map(scale => (
                          <button key={scale.id} onClick={async () => {
                            if(user.isDemo) { setProfile({...profile, uiScale: scale.id}); return; }
                            await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'core'), { uiScale: scale.id });
                          }} className={`p-6 rounded-2xl font-black text-[9px] transition-all border-[0.5px] tracking-widest ${profile.uiScale === scale.id ? 'border-red-600 text-red-500 bg-red-600/5' : 'border-white/5 text-zinc-700 bg-white/[0.01] hover:border-white/10'}`}>{String(scale.label).toUpperCase()}</button>
                        ))}
                      </div>
                    </div>
