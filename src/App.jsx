...
import { 
  ...
  Loader2 // تم إضافة الأيقونة المفقودة هنا
} from 'lucide-react';
      ...
      <div className="flex-1 overflow-hidden text-right" dir="rtl">
        <div className="flex items-center gap-2">
           ...
           {tk.deadline && tk.status !== 'done' && (
             <div className={`text-[8px] px-1.5 py-0.5 rounded-full font-black border-[0.5px] whitespace-nowrap ${isMissed ? 'bg-red-600/10 border-red-500/30 text-red-500' : 'bg-blue-600/10 border-blue-500/30 text-blue-500'}`}>
...
const App = () => {
  ...
  const [profile, setProfile] = useState({ name: "", aura: 0, lang: "ar_jo", streak: 0, lastLogin: "", waterLevel: 0, uiScale: 'default', lastWaterAmount: 0 });
  ...
  const glowX = useSpring(useTransform(mouseX, [0, 1920], [-50, 50]), { damping: 30, stiffness: 200 });
  const glowY = useSpring(useTransform(mouseY, [0, 1080], [-50, 50]), { damping: 30, stiffness: 200 });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => { setAiAdvice(quotes[Math.floor(Math.random() * quotes.length)]); }, 12000);
  ...
  useEffect(() => {
    ...
    const unsubP = onSnapshot(pRef, (snap) => {
      if (snap.exists()) {
        const data = snap.data(); setProfile(prev => ({ ...prev, ...data }));
    ...
    const unsubT = onSnapshot(tCol, (snap) => { setTasks(snap.docs.map(d => ({ id: d.id, ...d.data() }))); }, (err) => { });
    return () => { unsubP(); unsubT(); };
  }, [user, authChecking, appId]);

  const addWater = async (ml) => {
    const amount = parseInt(ml); if (isNaN(amount) || !user) return;
    const newLvl = (profile.waterLevel || 0) + amount;
    if (user.isDemo) { setProfile({...profile, waterLevel: newLvl}); setCustomWater(""); return; }
    await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'core'), { waterLevel: newLvl, lastWaterAmount: amount }, { merge: true });
    setCustomWater("");
  };
  ...
  const updateStatus = async (tk) => {
    ...
    if (next === 'done') await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'core'), { aura: (profile.aura || 0) + 10, hasDoneTaskToday: true }, { merge: true });
  };
  ...
  const handleUpdateName = async (newName) => {
    const updates = { name: String(newName) }; setProfile(prev => ({ ...prev, ...updates })); 
        ...
        <div className="w-full max-w-md bg-white/[0.01] backdrop-blur-3xl border-[0.5px] border-white/10 rounded-[3rem] p-10 shadow-2xl relative overflow-hidden glass-noise">
          ...
          {user ? (
             <div className="space-y-6">
                ...
                <button onClick={async () => { if (!tempName.trim()) return; await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'core'), { name: String(tempName), aura: 0, lang: 'ar_jo', streak: 0, uiScale: 'default', lastLogin: new Date().toISOString().split('T')[0], hasDoneTaskToday: false, waterLevel: 0 }, { merge: true }); }} className="w-full bg-red-600 py-6 rounded-2xl font-black text-white shadow-xl text-lg">تثبيت الهوية ⚡</button>
             </div>
          ) : (
            <div className="space-y-6">
  ...
  return (
    <div className={`min-h-screen bg-[#050505] text-white p-4 md:p-12 font-sans relative overflow-x-hidden ${s.text}`} dir={isArabic ? 'rtl' : 'ltr'}>
      <motion.div style={{ x: glowX, y: glowY }} className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[100vw] h-[100vw] bg-red-600/[0.02] blur-[150px] rounded-full pointer-events-none -z-10" />
      <div className="max-w-7xl mx-auto space-y-12 pb-40">
        <header className={`flex flex-col md:flex-row items-center justify-between gap-8 pb-10 border-b border-white/5 ${isArabic ? 'text-right' : 'text-left md:items-start'}`}>
          <div className="flex-1 w-full">
            <motion.h1 initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className={`${s.welcome} font-black italic tracking-tighter leading-none`}>
              {String(t.welcome.replace('{name}', ''))} <span className="text-red-600 drop-shadow-[0_0_20px_rgba(220,38,38,0.4)] uppercase font-black"> {String(profile.name)}</span>
            </motion.h1>
            <div className={`flex items-center gap-4 mt-4 h-8 overflow-hidden ${isArabic ? 'justify-start' : 'justify-start'}`}>
              <AnimatePresence mode='wait'><motion.p key={aiAdvice} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} transition={{ duration: 0.3 }} className="font-light text-zinc-500 italic max-w-2xl truncate text-base">"{String(aiAdvice)}"</motion.p></AnimatePresence>
            </div>
          </div>
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: [0.95, 1.05, 0.95] }} transition={{ repeat: Infinity, duration: 4 }} className="flex items-center gap-4 bg-white/[0.01] p-3 md:p-4 rounded-[3rem] border-[0.5px] border-white/10 backdrop-blur-3xl shadow-2xl">
             <div className="flex items-center gap-3 px-8 border-l border-white/5"><Flame size={s.icon + 10} className={profile.streak > 0 ? "text-orange-500 drop-shadow-[0_0_15px_rgba(249,115,22,0.5)]" : "text-zinc-800"} /><span className="text-5xl font-black tracking-tighter">{profile.streak || 0}</span></div>
        ...
        <div className="flex flex-col md:flex-row gap-6 max-w-5xl mx-auto">
           <div className="flex-1 bg-white/[0.02] p-4 rounded-[3rem] border-[0.5px] border-white/10 shadow-2xl backdrop-blur-3xl focus-within:ring-1 ring-red-600/10 transition-all glass-noise">
              <div className="flex items-center gap-4">
                 <div className="flex-1 flex items-center px-6 bg-black/40 rounded-[2.2rem] border-[0.5px] border-white/10 min-h-[60px]"><Plus size={24} className="text-zinc-800" /><input value={newTaskText} onChange={(e)=>setNewTaskText(e.target.value)} onKeyDown={(e)=>e.key==='Enter' && handleAddTask()} placeholder={String(t.add_task)} className="bg-transparent flex-1 text-lg font-bold text-white outline-none py-4" /></div>
           ...
           <div className="flex gap-3 p-3 bg-white/[0.01] border-[0.5px] border-white/10 rounded-[2.5rem] backdrop-blur-xl">
              ...
              <select value={statusFilter} onChange={(e)=>setStatusFilter(e.target.value)} className="bg-transparent text-[10px] font-black uppercase text-zinc-500 outline-none cursor-pointer px-2"><option value="all" className="bg-[#09090b]">All Filters</option>{Object.entries(t.status).map(([k, v]) => <option key={k} value={k} className="bg-[#09090b]">{String(v)}</option>)}</select>
           </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-40">
           <AnimatePresence mode='popLayout'>{filteredTasks.length === 0 ? (<motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 0.1 }} className="col-span-full py-40 text-center border-2 border-dashed border-white/5 rounded-[4rem] flex flex-col items-center gap-6"><ZapOff size={80} className="text-zinc-800" /><p className="text-2xl font-black italic tracking-widest uppercase text-zinc-700">Clear</p></motion.div>) : filteredTasks.map((tk) => (<TaskCard key={tk.id} tk={tk} t={t} s={s} onUpdate={updateStatus} onDelete={(id) => user.isDemo ? setTasks(tasks.filter(t=>t.id!==id)) : deleteDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'tasks', id))} onEdit={(task) => setEditingTask(task)} />))}</AnimatePresence>
        </div>

        <div className="fixed bottom-10 right-10 z-50 flex flex-col items-end gap-6">
           <AnimatePresence>{showWater && (<motion.div initial={{ opacity: 0, scale: 0.8, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.8, y: 20 }} className="bg-zinc-950/98 border-[0.5px] border-white/20 p-8 rounded-[4rem] shadow-[0_30px_100px_rgba(0,0,0,1)] w-80 backdrop-blur-3xl overflow-hidden glass-noise"><div className="flex justify-between items-center mb-8 relative z-10 px-2 text-right" dir="rtl"><h4 className="font-black text-xl italic uppercase tracking-tighter text-blue-500">Hydration</h4><button onClick={()=>setShowWater(false)}><X size={20} className="text-zinc-600" /></button></div><div className="h-56 w-full bg-black/60 rounded-[2.5rem] relative overflow-hidden mb-8 border-[0.5px] border-white/10 shadow-inner"><motion.div animate={{ y: [0, -3, 0], x: [-1, 1, -1] }} transition={{ repeat: Infinity, duration: 15, ease: "easeInOut" }} className="absolute bottom-0 left-[-10%] w-[120%] bg-blue-600/20" style={{ height: `${Math.min(100, ((profile.waterLevel || 0) / 4000) * 100)}%` }} /><div className="absolute inset-0 flex items-center justify-center font-black text-6xl tracking-tighter opacity-70 z-10">{Math.round(((profile.waterLevel || 0) / 4000) * 100)}%</div></div><div className="flex items-center gap-2 mb-6 text-right" dir="rtl">{[100, 250, 600].map(v => (<button key={v} onClick={()=>addWater(v)} className="flex-1 bg-white/5 hover:bg-blue-600/10 py-4 rounded-2xl font-black text-[10px] transition-all border border-white/10">{v}مل</button>))}</div><div className="flex gap-2"><input type="number" value={customWater} onChange={(e)=>setCustomWater(e.target.value)} placeholder={String(t.water.custom)} className="flex-1 bg-black/60 border border-white/10 rounded-2xl p-4 text-xs font-black text-white outline-none focus:ring-1 ring-blue-600" /><button onClick={()=>addWater(customWater)} className="bg-blue-600 p-4 rounded-2xl shadow-lg active:scale-90 transition-transform"><Plus size={20}/></button><button onClick={async () => { const lastAmt = profile.lastWaterAmount || 0; const newLvl = Math.max(0, (profile.waterLevel || 0) - lastAmt); if (user.isDemo) setProfile({...profile, waterLevel: newLvl}); else await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'core'), { waterLevel: newLvl }); }} className="bg-zinc-800 p-4 rounded-2xl text-zinc-500 hover:text-red-500 transition-all"><Undo2 size={20}/></button></div></motion.div>)}</AnimatePresence>
        ...
        <AnimatePresence>{editingTask && (<div className="fixed inset-0 z-[200] flex items-center justify-center p-6" dir="rtl"><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black backdrop-blur-2xl z-0" onClick={()=>setEditingTask(null)} /><motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-lg bg-zinc-900/90 border-[0.5px] border-white/10 p-10 rounded-[4rem] shadow-2xl glass-noise z-10"><button onClick={()=>setEditingTask(null)} className="absolute top-10 right-10 text-zinc-600 hover:text-white transition-colors"><X size={28}/></button><h2 className="text-3xl font-black mb-10 italic text-red-600 uppercase text-right">Update Task</h2><div className="space-y-10 text-right"><input value={String(editingTask.text)} onChange={(e)=>setEditingTask({...editingTask, text: e.target.value})} className="w-full bg-black/40 border-[0.5px] border-white/10 rounded-2xl p-5 text-xl font-black text-white outline-none" /><div className="grid grid-cols-2 gap-6"><div className="flex gap-2 p-1.5 bg-black/40 rounded-2xl border-[0.5px] border-white/5">{['morning', 'day', 'night'].map(sl => (<button key={sl} onClick={()=>setEditingTask({...editingTask, slot: sl})} className={`flex-1 p-3 rounded-xl transition-all ${editingTask.slot === sl ? 'bg-red-600 text-white shadow-lg' : 'text-zinc-700'}`}>{sl === 'morning' ? <Sun size={14}/> : sl === 'night' ? <Moon size={14}/> : <CloudSun size={14}/>}</button>))}</div><select value={editingTask.category} onChange={(e)=>setEditingTask({...editingTask, category: e.target.value})} className="w-full bg-black/40 border-[0.5px] border-white/10 rounded-2xl p-4 text-[10px] font-black text-white outline-none"><option value="personal">Personal</option><option value="uni">University</option><option value="gym">Gym</option></select></div><button onClick={async ()=>{ if(user.isDemo) { setTasks(tasks.map(t=>t.id===editingTask.id?editingTask:t)); setEditingTask(null); } else { await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'tasks', editingTask.id), editingTask); setEditingTask(null); } }} className="w-full py-6 bg-red-600 rounded-[2.2rem] font-black text-white shadow-xl active:scale-95 transition-all">Apply Changes ⚡</button></div></motion.div></div>)}</AnimatePresence>

        <AnimatePresence>{showSettings && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6 text-right" dir="rtl"><motion.div initial={{ opacity: 0 }} animate={{ opacity: 0.6 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black backdrop-blur-2xl z-0" onClick={()=>setShowSettings(false)} /><motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="relative w-full max-w-2xl bg-white/[0.05] border-[0.5px] border-white/10 p-12 md:p-16 rounded-[4rem] md:rounded-[5rem] shadow-2xl backdrop-blur-3xl max-h-[90vh] overflow-y-auto no-scrollbar glass-noise z-10"><button onClick={()=>setShowSettings(false)} className="absolute top-10 right-10 text-zinc-600 hover:text-white transition-colors z-20"><X size={32}/></button><div className="space-y-16 text-left" dir="ltr"><h2 className="text-4xl font-black italic mb-12 text-red-600 tracking-tighter uppercase">System Config</h2><div className="space-y-16"><div className="space-y-8"><div className="flex items-center gap-3 text-zinc-500 border-b border-white/5 pb-4"><IDIcon size={20} className="text-red-600" /><label className="text-[10px] font-black uppercase tracking-[0.5em] block italic">Profile Settings</label></div><div className="bg-black/40 border-[0.5px] border-white/10 rounded-2xl p-4 flex items-center gap-4 group focus-within:border-red-600/40 transition-all"><UserIcon size={18} className="text-zinc-700" /><input type="text" value={profile.name} onChange={(e) => handleUpdateName(e.target.value)} placeholder="Your Name..." className="bg-transparent flex-1 text-zinc-100 font-bold outline-none placeholder:text-zinc-800" /></div></div><div className="space-y-8"><div className="flex items-center gap-3 text-zinc-500 border-b border-white/5 pb-4"><LanguagesIcon size={20} className="text-red-600" /><label className="text-[10px] font-black uppercase tracking-[0.5em] block italic">System Tone</label></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{SYSTEM_LANGS.map(lang => (<button key={lang.id} onClick={async () => { if(user.isDemo) { setProfile({...profile, lang: lang.id}); return; } await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'core'), { lang: lang.id }); }} className={`p-8 rounded-[2rem] font-black text-xs transition-all border-[0.5px] flex items-center justify-between px-10 ${profile.lang === lang.id ? 'border-red-600 text-white bg-red-600/5 shadow-[0_0_20px_rgba(220,38,38,0.1)]' : 'border-white/5 text-zinc-700 bg-white/[0.01] hover:bg-white/5'}`}><span>{String(lang.label)}</span>{profile.lang === lang.id && <CheckCircle2 size={20} className="text-red-500" />}</button>))}</div></div><div className="space-y-8"><div className="flex items-center gap-3 text-zinc-500 border-b border-white/5 pb-4"><Maximize2 size={20} className="text-red-600" /><label className="text-[10px] font-black uppercase tracking-[0.5em] block italic">Interface Scale</label></div><div className="grid grid-cols-3 gap-4">{UI_SCALES.map(scale => (<button key={scale.id} onClick={async () => { if(user.isDemo) { setProfile({...profile, uiScale: scale.id}); return; } await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'core'), { uiScale: scale.id }); }} className={`p-6 rounded-2xl font-black text-[9px] transition-all border-[0.5px] tracking-widest ${profile.uiScale === scale.id ? 'border-red-600 text-red-500 bg-red-600/5' : 'border-white/5 text-zinc-700 bg-white/[0.01] hover:border-white/10'}`}>{String(scale.label).toUpperCase()}</button>))}</div></div><button onClick={async ()=>{ if(user.isDemo) window.location.reload(); else await signOut(auth); }} className="w-full py-8 bg-zinc-950 border-[0.5px] border-red-600/20 text-red-500 rounded-3xl font-black uppercase tracking-[0.5em] shadow-xl hover:bg-red-600 hover:text-white transition-all active:scale-95 leading-none">End Session</button></div></div></motion.div></div>)}</AnimatePresence>
        ...
        <footer className="mt-64 pt-24 border-t border-white/5 flex flex-col items-center justify-center px-6 pb-24 opacity-60 gap-16 text-center">
          ...
          <div className="flex flex-col items-center group cursor-default">
             <div className="relative">
                <motion.p transition={{ duration: 0.1 }} whileHover={{ color: "#ef4444", textShadow: "0 0 35px rgba(239,68,68,1)" }} className={`text-xl md:text-2xl font-black tracking-[0.5em] uppercase leading-none text-zinc-900 transition-colors cursor-pointer select-none whitespace-nowrap`}>HUMAM TAIBEH</motion.p>
